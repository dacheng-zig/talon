...
`build.zig` 只导出 `talon` 一个 module。core/http 的方向边界由约定与 module 根目录隔离维持（Zig 禁止 `@import` 模块根目录之外的文件，core 误依赖 `src/http/` 自然编译失败），不再单设独立的 `addObject` 编译守卫。
...
zig build test           # 跑 talon test 二进制（core + http，全部单元 + 集成测试）
zig build run-http       # 手动验证 HTTP
zig build run-resp       # 手动验证自定义协议路径
```

`build.zig` 建一个 `addTest`（`talon_mod`，整个 talon 模块 = core + http，覆盖全部单元测试）与一个 `tests/all.zig` 集成测试，`test` step 依赖两者。新增源文件后，记得在对应入口文件（`core/core.zig` 或 `http/http.zig`）的 `test {}` 块里 `_ = @import("...")`，否则 `refAllDecls` 扫不到、测试不会被编译。
